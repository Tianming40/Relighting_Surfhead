...
def brdf_training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint=True):
    ...
    for i, param_group in enumerate(gaussians.optimizer.param_groups):
        ...
        optimizer_params.append(group_info)
    ...
    mkdir_p(os.path.join(args.model_path, "relighting"))
    with open(os.path.join(args.model_path, "relighting/config.json"), "w") as f:
    ...
    for iteration in range(1, opt.brdf_iterations + 1):
        ...
        with torch.no_grad():
            ...
            if (iteration in checkpoint_iterations):
                ...
                torch.save((gaussians.capture(), iteration), scene.model_path + "relighting/chkpnt" + str(iteration) + ".pth")
